<template>
  <div class="template-list-component">
    ...
    <a-row :gutter="16" v-else>
      <a-col
        :span="6"
        v-for="item in listWithBarcode"
        :key="item.id"
        class="poster-item"
      >
        <a-card
          hoverable
          @mouseenter="() => showBarcode(item.id, item.barcodeUrl)"
        >
              ...
              <template v-slot:overlay>
                <a-menu class="overlay-dropdown">
                  ...
                  <a-menu-item v-if="item.coverImg">
                    <a
                      href="javascript:;"
                      @click.prevent="saveImage(item.coverImg)"
                      ><DownloadOutlined /> 下载图片</a
                    >
                  </a-menu-item>
                  <a-menu-item>
...
import { PropType, computed, ref, nextTick, reactive, Ref, watch } from "vue"
import {
  EditOutlined,
  BarChartOutlined,
  EllipsisOutlined,
...
const emits = defineEmits(["on-copy", "on-delete", "on-static", "on-send"])
...
const sendGift = () => {
  publishForm.value.validate().then(() => {
    emits("on-send", { id: currentItem.value, username: form.username })
  })
}
</script>
